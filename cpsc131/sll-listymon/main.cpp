...
void displayPokemon(const Pokemon& pokemon) {
    std::cout << "Name: " << pokemon.getName() << ", Type: " << pokemon.getType() << std::endl << std::endl;
}
...
int main() {
  ...
  //BONUS WAY 
  auto pkmn_itr = std::find(pokemonList.begin(), pokemonList.end(), Pokemon{"Charizard", "Fire"});
    if (pkmn_itr != pokemonList.end()) {
    std::cout << "Found without a loop!\n";
    displayPokemon(*pkmn_itr);
  }
